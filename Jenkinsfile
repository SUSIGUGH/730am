...
sh 'ssh -i jmtksrv01.pem -o StrictHostKeyChecking=no ec2-user@15.206.94.237 "cd /home/ec2-user && kubectl delete pod susimysql --force -n dev && kubectl create -f mysql.yaml && kubectl get pods -n dev"'
...
sh 'ssh -i jmtksrv01.pem -o StrictHostKeyChecking=no ec2-user@15.206.94.237 "cd /home/ec2-user && kubectl delete deployment php --force -n dev && kubectl create -f php.yaml && kubectl get deployments -n dev"'
sh 'ssh -i jmtksrv01.pem -o StrictHostKeyChecking=no ec2-user@15.206.94.237 "cd /home/ec2-user && kubectl delete service php-srv -n dev && kubectl create -f phpsrv.yaml && kubectl get deployments -n dev"'
sh 'ssh -i jmtksrv01.pem -o StrictHostKeyChecking=no ec2-user@15.206.94.237 "nohup kubectl port-forward --address 0.0.0.0 services/php-srv 30034:80 &"'
}
}
